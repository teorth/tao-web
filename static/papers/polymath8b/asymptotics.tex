...
$$ f(x) := 1_{x \leq 1-\eps} \int_0^{1+\eps-x} F(x,t)\ dt.$$
In the regime $0 < \eps < 1/3$, one can calculate that $f$ will (up to scalar multiples) take the form
\begin{align*}
 f(x) &= 1_{x \leq 2\eps} \frac{C_1}{\lambda-1-\eps+x} \\
&\quad + 1_{2\eps \leq x \leq 1-\eps} \left(\frac{\log(\lambda-x)-\log(\lambda-1-\eps+x)}{2\lambda-1-\eps} + \frac{1}{\lambda-1-\eps+x} \right)
\end{align*}
where
...
$$ f(x) = \frac{1_{x \leq 1-\eps}}{\lambda - 1 - \eps + x}$$
and 
...
for $1/3 \leq \eps < 1$.  In particular, $M_{2,\eps}$ increases to $2$ in the limit $\eps \to 1$; the lower bound $\liminf_{\eps \to 1} M_{2,\eps} \geq 2$ can also be established by testing with the function $F(x,y) := 1_{x \leq \delta, y \leq 1+\eps-\delta} + 1_{y \leq \delta, x \leq 1+\eps-\delta}$ for some sufficiently small $\delta>0$.
...
\begin{equation}\label{hdef}
 h(t) \coloneqq \frac{1}{r-S_{k-1}+(k-1)t} 1_{S_{k-1} < r}
\end{equation}
...
$$
\P( S_k > 1+\tau) \leq \frac{k\sigma^2}{(1+\tau-k\mu)^2}.
$$
But, from \eqref{mu-def}, \eqref{sigma-def}, we see that each $X_i$ has mean $\mu$ and variance $\sigma^2$, so $S_k$ has mean $k\mu$ and variance $k\sigma^2$.  The claim now follows from Chebyshev's inequality and \eqref{tau-bound}.
...
$$ \frac{\log k}{m_2} c^2 \E \int_{[0,T]} g(s)^2 \left(\int_0^1 \frac{1_{1+u\tau-S_{k-1} \geq s}}{1+u\tau-S_{k-1}+(k-1)s}\ du\right)\ ds.$$
