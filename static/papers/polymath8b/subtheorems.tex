...
\item[(iv)] $\DHL[\num{75845707},5]$.
\item[(v)] $\DHL[\num{3473955908},6]$.
\item[(vi)] $\DHL[k,m+1]$ whenever $m \geq 1$ and $k \geq C\exp( (4 - \frac{24}{181}) m )$ for some sufficiently large absolute (and effective) constant $C$.
\end{itemize}
...
We prove Theorem \ref{main-dhl} in Appendix \ref{tuples-sec}.  In the opposite direction, an application of the Brun-Titchmarsh theorem gives $H(k) \geq (\frac{1}{2} + o(1)) k \log k$ as $k \to \infty$; see \cite[\S 3.9]{polymath8a-unabridged} for this bound, as well as with some slight refinements.

\begin{table}
\centering
\setlength{\extrarowheight}{2pt}
\caption{Results used to prove various components of Theorem \ref{main-dhl}. Note that Theorems \ref{maynard-thm}, \ref{maynard-trunc}, \ref{epsilon-trick}, \ref{epsilon-beyond} are in turn proven using Theorems \ref{prime-asym}, \ref{nonprime-asym}.}\label{ingredients}
\begin{tabular}{lr}
\toprule
Theorem \ref{main-dhl}                 & Results used \\
\midrule
(i) & Lemma \ref{crit}, Theorems \ref{bv-thm}, \ref{epsilon-trick}, \ref{mke-lower} \\
(ii)-(vi) & Lemma \ref{crit}, Theorems \ref{mpz-poly}, \ref{maynard-trunc}, \ref{mlower-var} \\
(vii)-(xi) & Lemma \ref{crit}, Theorems \ref{maynard-thm}, \ref{maynard-trunc} \\
(xii) & Lemma \ref{crit}, Theorems \ref{epsilon-beyond}, \ref{piece} \\
(xiii) & Lemma \ref{crit}, Theorems \ref{epsilon-trick}, \ref{mke-lower} \\
\bottomrule
\end{tabular}
\end{table}
...
$$ m \leq (\log k - C) \left(\frac{1}{4} + \frac{6}{700} - \frac{C}{\log k}\right)$$
which is in particular implied by
%$$ m \leq \frac{\log k}{4 - \frac{52}{283}} - C'$$
$$ m \leq \frac{\log k}{4 - \frac{24}{181}} - C'$$
...
We prove this theorem in Section \ref{trick-sec}.  Parts (i), (xiii) of Theorem \ref{main-dhl}, and a weaker version $\DHL[4,2]$ of part (xii), then follow\footnote{Strictly speaking, one needs to subtract an infinitesimal amount from $1/50$ in Theorem \ref{mke-lower}(xiii) in order for Theorem \ref{epsilon-trick} to apply.} from Theorem \ref{bv-thm} and the following computations, proven in Sections \ref{mkeps-sec}, \ref{4d}:
