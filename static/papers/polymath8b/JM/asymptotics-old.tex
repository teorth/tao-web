...
$$ \left(\int_0^\infty F(t_1,\dots,t_k)\ dt_i\right)^2 \leq \frac{1}{k-1} \log(2k-1) \int_0^\infty \Bigl(1-\sum_{j\ne i}t_j\Bigr) F(t_1,\dots,t_k)^2\ dt_i.$$
...
Z_3 &\coloneqq \frac{1}{m_2} \int_0^T kt \log\Bigl(1+\frac{t}{T}\Bigr) g(t)^2\ dt \label{z3-def}\\
W &\coloneqq \frac{1}{m_2} \int_0^T \log\Bigl(1+\frac{\tau}{kt}\Bigr) g(t)^2\ dt\label{W-def}\\
...
where $r>1$ is a parameter which we will eventually average over.  We thus have (extending $g$ to $(0,\infty)$ by zero)
...
$$ J_k(F) = m_2^{k-1} \int_0^\infty \dots \int_0^\infty \Bigl(\int_{[0,r-t_1-\dots-t_{k-1}]} g(t)\ dt\Bigr)^2 \prod_{i=1}^{k-1} \frac{g(t_i)^2\ dt_i}{m_2},$$
where we adopt the convention that $\int_{[a,b]}$ vanishes when $b<a$.  In probabilistic language, we thus have
$$ J_k(F) = m_2^{k-1} \E \left(\Bigl(\int_{[0,r-X_1-\dots-X_{k-1}]} g(t)\ dt\Bigr)^2\right).$$
Also by symmetry we see that $J_i(F)=J_k(F)$ for all $i=1,\dots,k$.  Putting all this together, we conclude that
$$\E \left(\Bigl(\int_0^{r-X_1-\dots-X_{k-1}} g(t)\ dt\Bigr)^2\right) \leq \frac{m_2 M^{[T]}_k r}{k} \P( X_1 + \dots + X_k \geq r )$$
for all $r>1$.  Writing $S_i \coloneqq X_1 + \dots + X_i$, we abbreviate this as
\begin{equation}\label{rg1}
\E \left(\Bigl(\int_{[0,r-S_{k-1}]} g(t)\ dt\Bigr)^2\right) \leq \frac{m_2 M_k^{[T]} r}{k} \P( S_k \geq r ).
\end{equation}
Now we run a variant of the Cauchy-Schwarz argument used to prove Corollary \ref{mk-upper}.  For fixed $r>0$, we introduce the random function $h: (0,+\infty) \to \R$ by the formula
\begin{equation}\label{hdef}
 h(t) \coloneqq \frac{1}{r-S_{k-1}+(k-1)t} 1_{S_{k-1} \leq r}.
\end{equation}
We observe that whenever $S_{k-1} < r$, we have
\begin{equation}\label{h-int}
 \int_{[0,r-S_{k-1}]} h(t)\ dt = \frac{\log k}{k-1}.
\end{equation}
Thus, by the Legendre identity, we have
\[\begin{split} \left(\int_{[0,r-S_{k-1}]} g(t)\ dt\right)^2 &= \frac{\log k}{k-1} \int_{[0,r-S_{k-1}]} \frac{g(t)^2}{h(t)}\ dt \\
&\qquad- \frac{1}{2} \int_{[0,r-S_{k-1}]} \int_{[0,r-S_{k-1}]} \frac{(g(s)h(t)-g(t)h(s))^2}{h(s)h(t)}\ ds dt\end{split}\]
...
\begin{align*}
\E \int_{[0,r-S_{k-1}]} \frac{g(t)^2}{h(t)}\ dt &= m_2 \E (r - S_{k-1} + (k-1) X_k) 1_{X_k \leq r- S_{k-1}}\\
...
\[\begin{split}
 \E (\int_{[0,r-S_{k-1}]} g(t)\ dt)^2 &= \frac{\log k}{k-1} m_2 r \P(S_k \leq r)\\
&\qquad - \frac{1}{2} \E \int_{[0,r-S_{k-1}]} \int_{[0,r-S_{k-1}]} \frac{(g(s)h(t)-g(t)h(s))^2}{h(s)h(t)}\ ds dt.
\end{split}\]
...
Thus, thanks to \eqref{ksb}, to prove \eqref{bigbound} it suffices to establish the bounds
...
$$
But, from \eqref{mu-def}, \eqref{sigma-def}, we see that each $X_i$ has mean $\mu$ and variance $\sigma^2$, so $S_k$ has mean $k\mu$ and variance $k\sigma^2$.  The claim now follows from \eqref{tau-bound} and Chebyshev's inequality.
...
$$ \int_{[T,r-S_{k-1}]} h(s)\ ds \leq \frac{1}{k-1} \log_+ \Bigl(\frac{r-S_{k-1}}{T}\Bigr),$$
where $\log_+(x) \coloneqq \max(\log x, 0)$.  We conclude that
$$ Y_1(r) \leq \frac{k}{k-1} \frac{1}{m_2} \E \int_{[0,T]} \frac{g(t)^2}{h(t)}\ dt \log_+ \Big(\frac{r-S_{k-1}}{T}\Bigr).$$
We can rewrite this as
$$ Y_1(r) \leq \frac{k}{k-1} \E \frac{1_{S_k \leq r}}{h(X_k)} \log_+\Bigl( \frac{r-S_{k-1}}{T}\Bigr).$$
...
$$ \log_+ \Bigl(\frac{r-S_{k-1}}{T}\Bigr) \leq \log_+ \Bigl(\frac{r-S_{k}}{T}\Bigr) + \log\Bigl(1+\frac{X_k}{T}\Bigr).$$
We conclude that
\begin{align*}
Y_1(r) &\leq \frac{k}{k-1} \E (r-S_k+kX_k) \left( \log_+ \Bigl(\frac{r-S_{k}}{T}\Bigr) + \log\Bigl(1+\frac{X_k}{T}\Bigr) \right) 1_{S_k \leq r}\\
&\leq \frac{k}{k-1} \E \Biggl(  (r-S_k+kX_k) \log_+\Bigl( \frac{r-S_{k}}{T}\Bigr) + (r-S_k)1_{S_k \leq r} \frac{X_k}{T} \\
&\qquad + k X_k \log\Bigl(1+\frac{X_k}{T}\Bigr) \Biggr).
\end{align*}
Here we used the elementary bound $\log(1+y) \leq y$.  Symmetrizing in the $X_1,\dots,X_k$, we conclude that
\begin{equation}\label{y1r}
 Y_1(r) \leq \frac{k}{k-1} (Z_1(r) + Z_2(r) + Z_3),
\end{equation}
where
\begin{align*}
Z_1(r) &\coloneqq \E r \log_+ \Bigl(\frac{r-S_k}{T}\Bigr), \\
Z_2(r) &\coloneqq \E (r-S_k) 1_{S_k \leq r} \frac{S_k}{kT} ,
\end{align*}
and $Z_3$ was defined in \eqref{z3-def}.

For the minor error term $Z_2$, we use the crude bound $(r-S_k) S_k1_{S_k \leq r} \leq \frac{r^2}{4}$, so
...
$$ \log_+ \Bigl(\frac{r-S_k}{T}\Bigr) \leq \frac{(r-S_k-2aT\log a-aT)^2}{4a^2 T^2 \log a}.$$
...
Z_1(r) \leq r \Bigl( \log \frac{r-k\mu}{T} + \frac{k\sigma^2}{4(r-k\mu)^2 \log \frac{r-k\mu}{T}}\Bigr).
\end{equation}
...
for $t \in [0, \min(r-S_{k-1},T)]$.  Thus, letting $\mathcal{I}$ denote the interval $[0, \min(r-S_{k-1},T)]$, we have
\begin{align*}
Y_2(r) &= \frac{k}{2 m_2} \E \int_{\mathcal{I}} \int_{\mathcal{I}} \frac{((g-h)(s) h(t)-(g-h)(t) h(s))^2}{h(s) h(t)}\ ds dt \\
&= \frac{k}{2 m_2} \E (r - S_{k-1} - c)^2 \int_{\mathcal{I}} \int_{\mathcal{I}} (g(s)-g(t))^2 h(s) h(t)\ ds dt.
\end{align*}
Using the crude bound $(g(s)-g(t))^2 \leq g(s)^2+g(t)^2$ and using symmetry, we conclude
$$Y_2(r) \leq \frac{k}{m_2} \E (r - S_{k-1} - c)^2 \int_{\mathcal{I}} \int_{\mathcal{I}} g(s)^2 h(s) h(t)\ ds dt.$$
...
$$ Q(u) \coloneqq (1+u\tau - S_{k-1} - c)^2 \int_{[0,\min(1+u\tau-S_{k-1},T)]} \frac{g(s)^2 ds}{1+u\tau-S_{k-1}+(k-1)s}.$$
...
$$ \frac{\log k}{m_2} c^2 \E \int_{[0,T]} g(s)^2 \Bigl(\int_0^1 \frac{1_{1+a\tau-S_{k-1} \geq s}}{1+u\tau-S_{k-1}+(k-1)s}\ du\Bigr)\ ds.$$
...
&\leq \frac{1}{\tau} \log \Bigl(1+\frac{\tau}{ks}\Bigr)
\end{align*}
...
\begin{align*}
m_2 &= \frac{1}{k-1} \Bigl( \frac{1}{c} - \frac{1}{c+(k-1)T} \Bigr) \\
&= \frac{\log k}{k} \Bigl(1 - \frac{\alpha}{\log k} + o\Bigl( \frac{1}{\log k} \Bigr) \Bigr),
\end{align*}
...
&= \frac{1}{k-1} \log \Bigl(\frac{c+(k-1)T}{c}\Bigr) \\
&= \frac{\log k}{k} \Bigl(1 + \frac{\log \beta}{\log k} + o\Bigl( \frac{1}{\log k} \Bigr) \Bigr).
\end{align*}
Thus
\begin{align*}
 k\mu &= \frac{k}{k-1} \Bigl(1 + \frac{\log \beta + \alpha}{\log k} + o\Bigl(\frac{1}{\log k} \Bigr) \Bigr) - \frac{kc}{k-1} \\
&= 1 + \frac{\log \beta + \alpha - 1}{\log k} + o\Bigl( \frac{1}{\log k} \Bigr).
\end{align*}
Similarly, from \eqref{g-def}, \eqref{mu-def} and \eqref{sigma-def} we have
\begin{align*}
...
and thus
\begin{align*}
k \sigma^2 &= \frac{k}{(k-1)^2} \Bigl(\frac{T}{m_2} - c^2 - 2c(k-1)\mu\Bigr) - k \mu^2 \\
&= \frac{\beta}{\log^2 k} + o\Bigl( \frac{1}{\log^2 k} \Bigr).
\end{align*}
...
In all cases the constraints \eqref{tau-bound}-\eqref{ksb} hold for the given choice of parameters.
