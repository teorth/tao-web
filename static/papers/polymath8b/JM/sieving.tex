...
%% JM: Adding modification to allow coprime to an integer
 and let $N$ be an integer coprime to $W$ with $\log{N}=O(\log^{O(1)}{x})$. Let $F_1,\dots,F_k,G_1,\dots,G_k: [0,+\infty)\to \R$ be smooth compactly supported functions.  Then
\begin{equation}\label{multisum}
 \sum_{\substack{d_1,\dots,d_k,d'_1,\dots,d'_k \\ [d_1,d'_1],\dots,[d_k,d'_k], W \text{ coprime}}} \prod_{j=1}^k \frac{\mu(d_j) \mu(d'_j) F_j( \log_x d_j ) G_j( \log_x d'_j)}{[d_j,d'_j]} = (c+o(1)) B^{-k}\frac{N^k}{\phi(N)^k}
\end{equation}
...
\begin{equation}\label{kp-est}
 K_p(\xi_1,\dots,\xi_k,\xi'_1,\dots,\xi'_k) = \Bigl(1+O(\frac{1}{p^2})\Bigr) \prod_{j=1}^k \frac{(1 - p^{-1-\frac{1+i\xi_j}{\log x}})(1 - p^{-1-\frac{1+i\xi'_j}{\log x}})}{1 - p^{-1-\frac{2+i\xi_j+i\xi'_j}{\log x}}}.
\end{equation}
Since 
$$ 
...
and so by Lemma \ref{mul-asym}, the contribution of the main term in \eqref{ts} is $(c+o(1)) B^{1-k} \frac{x}{\phi(W)}$.  By \eqref{W-bound} and \eqref{bnorm}, it thus suffices to show that for any fixed $A$
...
To proceed further, we will need some further information on the divisor sums $\lambda_F$ (defined in \eqref{lambdaf-def}), namely that these sums are concentrated on ``almost primes''; results of this type have also appeared in \cite{pintz-szemeredi}.
...
\sum_{x \leq n \leq 2x: n = b\ (W)} \prod_{i=1}^k \Bigl(|\lambda_{F_j}(n+h_i)|^{a_j} \tau(n+h_j)^{m_j}\Bigr) 1_{p_0|n+h_{j_0}} \ll \frac{\log_x p_0}{p_0} B^{-k} \frac{x}{W}.
\end{equation}
As a consequence, we have
\begin{equation}\label{lambdatau-fix2}
\sum_{x \leq n \leq 2x: n = b\ (W)} \prod_{i=1}^k \Bigl(|\lambda_{F_j}(n+h_i)|^{a_j} \tau(n+h_j)^{m_j}\Bigr) 1_{p(n+h_{j_0}) \leq x^\eps} \ll \eps B^{-k} \frac{x}{W},
\end{equation}
...
$$ \lambda_{F_j}(n) = \int_\R \Bigl(\sum_{d|n} \mu(d) \frac{1}{d^{\frac{1+i\xi}{\log x}}}\Bigr) f_j(\xi)\ d\xi, $$
...
since the product is $G(0)^k=1$ if $p( \frac{n+h_j}{d_j}) \geq R$, and non-negative otherwise. The right hand side may be expanded as
...
The $O(1)$ term contributes $\lessapprox R^k\lessapprox x^{1/10}$, which is negligible. By Lemma \ref{mul-asym}, if $\Omega(d)\ll \log{x}$ then the main term contributes
$$\ll \Bigl(\frac{d}{\phi(d)}\Bigr)^k\frac{x}{d W}(\log{R})^{-k}\ll 2^{\Omega(d)}B^{-k}\frac{x}{d W}.$$
We see that this final bound applies trivially if $\Omega(d)\gg \log{x}$. The bound \eqref{xnx} thus reduces to
...
$$
 \sum_{\substack{x \leq n \leq 2x\\ n = b\ (W)}} \Bigl(\prod_{i=1}^k \lambda_{F_{i}}(n+h_{i}) \lambda_{G_{i}}(n+h_{i})\Bigr) 1_{p(n+h_k) \leq x^\eps} = O( \eps B^{-k} \frac{x}{W} )
$$
