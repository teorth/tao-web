...
Suppose that for each fixed admissible $k$-tuple $(h_1,\dots,h_k)$
  and each congruence class $b\ (W)$ such that $b+h_i$ is coprime to $W$ for all $i=1,\dots,k$, one can find a non-negative weight
...
Let $(h_1,\ldots,h_{k})$ be a fixed
  ...
  one congruence class $b\ (W)$ such that $(b+h_i,W)=1$ for all $h_i
...
In the converse direction, in ??? we will also show the upper bound $M_k \leq \frac{k}{k-1} \log k$ for all $k \geq 2$, which shown in particular that the bounds in (vii) and (xi) of the above theorem cannot be significantly improved.  We also remark that Theorem \ref{mlower}(vii) also gives a weaker version $\DHL[54,2]$ of Theorem \ref{main-dhl}(i).  Finally, 
