...
\begin{equation}\label{tidef}
 \tilde I(f_3) := \int_{[0,+\infty)^k} |\frac{\partial^k}{\partial t_1 \dots \partial t_k} f_3(t_1,\dots,t_k)|^2\ dt_1 \dots dt_k
\end{equation}
and
\begin{equation}\label{tjdef}
 \tilde J_i(f_3) := \int_{[0,+\infty)^k} |\frac{\partial^k}{\partial t_1 \dots \partial t_k} f_3(t_1,\dots,t_{i-1}, 0, t_{i+1}, \dots, t_k)|^2\ dt_1 \dots dt_{i-1} dt_{i+1} \dots dt_k.
\end{equation}
In particular, 
...
\end{equation}
with $f_4$ not identically zero, with
...
\begin{align*}
\alpha &= \int_{[0,+\infty)^k} |\frac{\partial^k}{\partial t_1 \dots \partial t_k} f_4(t_1,\dots,t_k)|^2\ dt_1 \dots dt_k \\
&= \tilde I(f_4).
\end{align*}
...
\begin{align*}
\beta_k &= \int_{[0,+\infty)^k} |\frac{\partial^k}{\partial t_1 \dots \partial t_{k-1}} f_4(t_1,\dots,t_{k-1},0)|^2\ dt_1 \dots dt_{k-1} \\
&= \tilde J_k(f_4).
\end{align*}
...
\end{equation}
with
$$ \tilde J_{i, (1-\eps) \frac{\vartheta}{2}}(f_4) := \int_{(1-\eps) \frac{\vartheta}{2} \cdot {\mathcal R}_k} |\frac{\partial^k}{\partial t_1 \dots \partial t_k} f_4(t_1,\dots,t_{i-1}, 0, t_{i+1}, \dots, t_k)|^2\ dt_1 \dots dt_{i-1} dt_{i+1} \dots dt_k,$$
and such that each component $f_{1,j}(t_1) \dots f_{k,j}(t_k)$ is supported in the region
$$ \{ (t_1,\dots,t_k) \in \R^k: \sum_{i=1}^k \max(t_i, \delta_2/8) \leq (1+\eps) \frac{\vartheta}{2} - \delta_2/8; t_1,\dots,t_k < \delta - \delta_2/8 \}$$
...
$$ (\prod_{i=1}^k \int_0^1 f'_{i,j}(t_i) f'_{i,j'}(t_i)\ dt_i + o(1)) B^{-k} \frac{x}{W}.$$
...
The point of performing this lower bound is that if $j \in {\mathcal J}_1 \cup {\mathcal J}_2$ and $j' \in {\mathcal J}_2$, then from \eqref{sff}, \eqref{sff-minus} one has
$$ \sum_{i=1}^{k-1} S(f_{i,j}) + S(f_{i,j-1}) < \vartheta$$
...
\begin{align*}
\beta_k &= \int_{[0,+\infty)^k} \left(\frac{\partial^k}{\partial t_1 \dots \partial t_{k-1}} f_{4,1}(t_1,\dots,t_{k-1},0) + 2\frac{\partial^k}{\partial t_1 \dots \partial t_{k-1}} f_{4,2}(t_1,\dots,t_{k-1},0)\right)\\
&\quad\quad  \frac{\partial^k}{\partial t_1 \dots \partial t_{k-1}} f_{4,1}(t_1,\dots,t_{k-1},0)\ dt_1 \dots dt_{k-1} 
\end{align*}
where
...
$$ \beta_k \geq \tilde J_{k, (1-\eps) \frac{\vartheta}{2}}( f_4 ) + O(\delta_3).$$
A similar argument gives
$$ 
 \sum_{\substack{x \leq n \leq 2x\\ n = b\ (W)}} \nu(n) \theta(n+h_i) \geq (\beta_i - o(1)) B^{1-k} \frac{x}{\phi(W)}$$
for $i=1,\dots,k$ with
$$ \beta_i \geq \tilde J_{i, (1-\eps) \frac{\vartheta}{2}}( f_4 ) + O(\delta_3).$$
...
Finally, we prove Theorem \ref{epsilon-trick}. Let $k, m, \eps, F$ be as in that theorem.  By rescaling as in previous sections, we may find
...
From \eqref{sfk-sum}, the hypothesis $\EH[\vartheta]$ (which is available by Proposition \ref{geh-eh}) and the argument from the previous section used to prove Theorem \ref{epsilon-trick}(ii), we have
$$ \sum_{\substack{x \leq n \leq 2x\\ n = b\ (W)}} \nu(n) = (\alpha + o(1)) B^{-k} \frac{x}{W}$$
where 
$$ \alpha := \tilde I(f_4).$$

Similarly, from \eqref{sfk-sum-2} (and the upper bound $S(f_{i,j}) < 1$ from \eqref{sfk-sum}), the hypothesis $\GEH[\vartheta]$, and the argument from the previous section we have
$$ 
 \sum_{\substack{x \leq n \leq 2x\\ n = b\ (W)}} \nu(n) \theta(n+h_i) \geq (\beta_i - o(1)) B^{1-k} \frac{x}{\phi(W)}$$
for $i=1,\dots,k$ with
$$ \beta_i \geq \tilde J_{i, (1-\eps) \frac{\vartheta}{2}}( f_4 ) + O(\delta_3).$$
